...
void villageCard (int currentPlayer, struct gameState *state, int handPos){
	...
	//+2 Actions
	//state->numActions = state->numActions + 2; //commented out

	//discard played card from hand
	discardCard(handPos, currentPlayer, state, 0);
}//end villageCard

void outPostCard(int currentPlayer, struct gameState *state, int handPos){
	
	//set outpost flag
	//state->outpostPlayed++; //Commented out
...
void smithyCard(int currentPlayer, struct gameState *state, int handPos){
        ...
        //+3 Cards
      int i=0;
      for (i = 0; i < 3; i++)//moved i<3 to i<2
      {
        drawCard(currentPlayer, state);
      }
...
int shuffle(int player, struct gameState *state) {
  ...
  while (state->deckCount[player] > 0) {
    ...
    //printf("state->deck[player][card]=%i\n",state->deck[player][card]);
    newDeckPos++;
...
{	int count;
  ...
  if (state->deckCount[player] <= 0){//Deck is empty
    ...
    state->deckCount[player]--;
    state->handCount[player]++;//Increment hand count
    //printf("handCount in drawCard: %i\n", state->handCount[player]);
  }
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case outpost:
		outPostCard(currentPlayer, state, handPos);
      return 0;
